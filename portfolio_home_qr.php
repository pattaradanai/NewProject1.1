...
<script src='jquery.reel.js' type='text/javascript'></script>
	...
	<nav class="fh5co-nav" role="navigation">
		<div class="container">
			...
			<div id="fh5co-logo"><a href="student.php">ย้อนกลับ</a></div>
			</div>
	...
	<div class='container'>
    <div align = "right" style = "padding-top: 5%;float:right" >
    ...
    <div align = "left" style='margin:5% 0px 0px 3em;float:left'>	
		<p style='padding:0px;margin:0px;color:black'>qr code สำหรับ portfolio นี้</p>
		<?php
			echo "<img id='barcode' 
			src='https://api.qrserver.com/v1/create-qr-code/?data=http://localhost/NewProject1.1/portfolio_home.php?stdid={$_GET['stdid']}&amp;size=100x100' 
			width='100' 
			height='100' 
            ...
            <div class="row top-line animate-box" style='padding-bottom:1em;'>
				...
				while($workid = $imgstd -> fetch_assoc()){
					...
					while($row = $query->fetch_assoc()){
						...
						echo "<a class='work' href='portfolio_show_work_qr.php?stdid=$studentid&wid={$workid['workid']}&subid={$subject['subjectid']}' name='studentid_form_index'>";
						...
						while($name_data = $name -> fetch_assoc()){
							if($name_data['sex']==0){
								echo "ผลงานของด.ช.{$name_data['name']} {$name_data['surname']}";
							}else{
								echo "ผลงานของด.ญ.{$name_data['name']} {$name_data['surname']}";
							}
							$count++;
							break;
						}		
